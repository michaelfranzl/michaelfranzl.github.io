...
class Base extends HTMLElement {
  constructor() {
    ...
    linkElem.setAttribute('href', new URL(new URL('assets/style-d3695d34.css', import.meta.url).href, import.meta.url).href);
  ...
  setSegments(labels) {
    ...
    Object.keys(this.segmentMap).forEach(label => {
      ...
      if (labels.indexOf(label) === -1) segment.classList.remove('on');else segment.classList.add('on');
    });
  }
  ...
  async initialize() {
    ...
    for (const [label, id] of Object.entries(this.segmentMap)) this.segments[label] = this.root.getElementById(id);
...
class Digit extends Base {
  constructor() {
    ...
    this.svgFilepath = new URL(new URL('assets/digit-643c51dc.svg', import.meta.url).href, import.meta.url);
    this.initialize();
  }
...
class Sign extends Base {
  constructor() {
    ...
    this.svgFilepath = new URL(new URL('assets/sign-8852e63e.svg', import.meta.url).href, import.meta.url);
    this.initialize();
  }
...
// SPDX-License-Identifier: GPL-2.0-or-later
// SPDX-FileCopyrightText: Copyright 2020 Michael Karl Franzl <public.michael@franzl.name>
function createChild$1(parent, tag, {
  classes = [],
  id,
  attributes = {}
} = {}) {
...
class DskyInterface extends HTMLElement {
  constructor() {
    ...
    this.rootEl = this.attachShadow({
      mode: 'closed'
    });
    const styleHref = new URL(new URL('assets/webDSKY-a2d07dde.css', import.meta.url).href, import.meta.url).href;
    createChild$1(this.rootEl, 'link', {
      attributes: {
        rel: 'stylesheet',
        href: styleHref
      }
    ...
    fetch(styleHref);
    fetch(new URL(new URL('assets/LM11-co46-dsky-bg-7d148d98.png', import.meta.url).href, import.meta.url).href);
    fetch(new URL(new URL('assets/LM11-co46-dsky-overlay-2a70b9b7.png', import.meta.url).href, import.meta.url).href);
    fetch(new URL(new URL('assets/digit-643c51dc.svg', import.meta.url).href, import.meta.url).href);
    fetch(new URL(new URL('assets/sign-8852e63e.svg', import.meta.url).href, import.meta.url).href);
  }

  connectedCallback() {
    const iface = createChild$1(this.rootEl, 'div', {
      classes: ['interface']
    });
    createChild$1(iface, 'div', {
      classes: ['overlay']
    });
    createChild$1(iface, 'div', {
      classes: ['digitContainer', 'blur']
    });
    const digitContainer = createChild$1(iface, 'div', {
    ...
    for (let row = 1; row <= 3; row++) {
      const div = createChild$1(digitContainer, 'div', {
        classes: ['digitRow', `row${row}`]
      });
      createChild$1(div, 'div', {
        classes: ['separator']
      });
      this.elements.ds1[`${row}sign`] = createChild$1(div, 'dsky-display-sign');

      for (let digit = 5; digit >= 1; digit--) this.elements.ds1[`${row}${digit}`] = createChild$1(div, 'dsky-display-digit');
    ...
    for (const row of [4, 5, 7]) {
      ...
      for (let digit = 1; digit <= 2; digit++) this.elements.ds1[`${row}${digit}`] = createChild$1(div, 'dsky-display-digit');
    } // create verb, noun, prog backlights


    for (const id of ['prog', 'verb', 'noun']) createChild$1(digitContainer, 'div', {
      classes: [`${id}_bg`]
    }); // create COMP ACTY backlight


    this.elements.ds1.comp_acty = createChild$1(digitContainer, 'div', {
      classes: ['acty_bg']
    });
    this.elements.ds1.comp_acty.style.display = 'none'; // create 7 rows and 2 columns of alarm indicator lights

    const lampContainer = createChild$1(this.rootEl, 'div', {
      id: 'lamps'
    });
    ...
    for (let row = 0; row < 7; row++) {
      for (let col = 0; col < 2; col++) {
        ...
        this.elements.ds2[id] = createChild$1(lampContainer, 'div', {
          id,
          classes: ['lamp']
        });
      }
    ...
    for (const key of keys) {
      const element = this.elements.keys[key] = createChild$1(this.rootEl, 'div', {
        id: `key_${key}`,
        classes: ['key']
      });
      ...
      element.addEventListener('mousedown', event => {
        ...
        if (char === 'o') this.dispatchEvent(new CustomEvent('proceed', {
          ...
          detail: DskyInterface.keyToValue(char)
        }));
      });
  ...
  handleLampsUpdate(value) {
    ...
    if (value & 1 << 0) console.log('AGC WARNING is on'); // Update COMP ACTY

    if (value & 1 << 1) this.elements.ds1.comp_acty.style.display = 'inherit';else this.elements.ds1.comp_acty.style.display = 'none';
  ...
  switchLamp(id, state) {
    ...
    if (state) element.classList.add('on');else element.classList.remove('on');
  }
  ...
  handleDigitUpdate(newVal) {
    ...
    for (const row of [1, 2, 3]) {
      ...
      let segments;
      if (!this.state[`sign${row}m`] && !this.state[`sign${row}p`]) segments = '';else if (this.state[`sign${row}p`] && !this.state[`sign${row}m`]) segments = 'abc';else segments = 'b';
...
var ja$1 = "undefined" !== typeof global ? global : "undefined" !== typeof self ? self : "undefined" !== typeof window ? window : {},
    m = [],
    u$1 = [],
    ka$1 = "undefined" !== typeof Uint8Array ? Uint8Array : Array,
    la$1 = !1;
...
var ra$1 = {}.toString,
    sa$1 = Array.isArray || function (a) {
  return "[object Array]" == ra$1.call(a);
};

v.TYPED_ARRAY_SUPPORT = void 0 !== ja$1.TYPED_ARRAY_SUPPORT ? ja$1.TYPED_ARRAY_SUPPORT : !0;
...
v.prototype.writeUInt16LE = function (a, b, c) {
  ...
  v.TYPED_ARRAY_SUPPORT ? (this[b] = a & 255, this[b + 1] = a >>> 8) : Ma$1(this, a, b, !0);
  return b + 2;
};
...
v.prototype.writeUInt16BE = function (a, b, c) {
  ...
  b |= 0;
  c || D(this, a, b, 2, 65535, 0);
  v.TYPED_ARRAY_SUPPORT ? (this[b] = a >>> 8, this[b + 1] = a & 255) : Ma$1(this, a, b, !1);
  return b + 2;
};
...
v.prototype.writeUInt32LE = function (a, b, c) {
  ...
  v.TYPED_ARRAY_SUPPORT ? (this[b + 3] = a >>> 24, this[b + 2] = a >>> 16, this[b + 1] = a >>> 8, this[b] = a & 255) : Na$1(this, a, b, !0);
  return b + 4;
};
...
v.prototype.writeUInt32BE = function (a, b, c) {
  ...
  b |= 0;
  c || D(this, a, b, 4, 4294967295, 0);
  v.TYPED_ARRAY_SUPPORT ? (this[b] = a >>> 24, this[b + 1] = a >>> 16, this[b + 2] = a >>> 8, this[b + 3] = a & 255) : Na$1(this, a, b, !1);
  return b + 4;
};
...
v.prototype.writeInt32LE = function (a, b, c) {
  ...
  v.TYPED_ARRAY_SUPPORT ? (this[b] = a & 255, this[b + 1] = a >>> 8, this[b + 2] = a >>> 16, this[b + 3] = a >>> 24) : Na$1(this, a, b, !0);
  return b + 4;
};
...
v.prototype.fill = function (a, b, c, d) {
  ...
  a || (a = 0);
  if ("number" === typeof a) for (d = b; d < c; ++d) this[d] = a;else for (a = z$1(a) ? a : Da$1(new v(a, d).toString()), e = a.length, d = 0; d < c - b; ++d) this[d + b] = a[d % e];
  return this;
};
...
var Ra$1 = Object.freeze({
  ...
  isBuffer: Ca$1
    ...
    E$1 = v,
    Sa$1 = "undefined" !== typeof globalThis ? globalThis : "undefined" !== typeof window ? window : "undefined" !== typeof global ? global : "undefined" !== typeof self ? self : {};
...
function Za() {
  I$1 && J$1 && (I$1 = !1, J$1.length ? H$1 = J$1.concat(H$1) : Ya = -1, H$1.length && $a());
}
...
var L$1 = ja$1.performance || {},
    cb$1 = L$1.now || L$1.mozNow || L$1.msNow || L$1.oNow || L$1.webkitNow || function () {
  ...
  function f(a, b, c, f) {
    if (!(l.isBuffer(a) || a instanceof Sa$1.Uint8Array)) throw new TypeError('"buf" argument must be a Buffer or Uint8Array');
  ...
  a.exports = gb$1;
    ...
    ib$1 = Math.floor(.001 * (Date.now() - performance.now()));
...
var kb$1 = {
  resolve: function () {
    for (var a = "", b = !1, c, d = arguments.length - 1; -1 <= d && !b; d--) {
      if (0 <= d) var e = arguments[d];else void 0 === c && (c = eb$1.cwd()), e = c;
      M(e);
  ...
  fs: null
    ...
    X$1 = k(134217728),
    Nb$1 = k(268435456),
    Ob$1 = O$1 | P$1 | Q$1 | R | S | qb | T | V$1 | sb$1 | tb$1 | ub$1 | vb$1 | wb$1 | xb$1 | yb$1 | zb$1 | Ab$1 | Bb$1 | Cb$1 | Db$1 | Eb$1 | W$1 | Jb$1 | Ib$1 | Kb$1 | Mb$1 | Lb$1 | X$1 | Nb$1,
    Pb$1 = O$1 | P$1 | Q$1 | R | S | qb | T | V$1 | sb$1 | W$1 | Ib$1 | Jb$1 | X$1,
    Qb$1 = k(0),
    Rb$1 = R | S | V$1 | tb$1 | ub$1 | vb$1 | wb$1 | xb$1 | yb$1 | zb$1 | Ab$1 | Bb$1 | Cb$1 | Db$1 | Eb$1 | W$1 | Jb$1 | Kb$1 | Mb$1 | Lb$1 | X$1,
    Sb$1 = Rb$1 | Pb$1,
    Tb$1 = P$1 | R | T | W$1 | X$1 | Nb$1,
    Ub$1 = P$1 | R | T | W$1 | X$1,
    Vb$1 = k(0),
    Wb = {
  E2BIG: 1,
  EACCES: 2,
  ...
  EXDEV: 75
    ...
    Yb = O$1 | P$1 | S | V$1 | W$1 | X$1,
    Zb$1 = O$1 | T | S | V$1 | W$1 | X$1;
...
var ac$1 = function (a) {
  ...
  function a(a) {
    ...
    function d(a, b) {
      a = bc$1(g, a);
      if (b !== k(0) && (a.rights.base & b) === k(0)) throw new ac$1(63);
      return a;
    }
    ...
    try {
      for (var ua = ca$1(Object.entries(h)), ea = ua.next(); !ea.done; ea = ua.next()) {
        var rb = da$1(ea.value, 2),
            fc = rb[0],
            Fb = rb[1],
    ...
    this.wasiImport = {
      args_get: function (a, b) {
        ...
        n.forEach(function (a) {
          ...
          c += 4;
          d += E$1.from(g.memory.buffer).write(a + "\x00", d);
        });
        return 0;
      },
      ...
      environ_get: function (a, b) {
        ...
        Object.entries(l).forEach(function (a) {
          ...
          g.view.setUint32(c, d, !0);
          c += 4;
          d += E$1.from(g.memory.buffer).write(a + "=" + b + "\x00", d);
        });
        return 0;
      },
      ...
      fd_advise: Z$1(function (a) {
        d(a, V$1);
        return 52;
      }),
      fd_allocate: Z$1(function (a) {
        d(a, sb$1);
        return 52;
      }),
      fd_close: Z$1(function (a) {
        var b = d(a, k(0));
        p.closeSync(b.real);
        g.FD_MAP.delete(a);
        return 0;
      }),
      fd_datasync: Z$1(function (a) {
        a = d(a, O$1);
        p.fdatasyncSync(a.real);
        return 0;
      }),
      fd_fdstat_get: Z$1(function (a, b) {
        a = d(a, k(0));
        g.refreshMemory();
      ...
      fd_fdstat_set_flags: Z$1(function (a) {
        d(a, R);
        return 52;
      }),
      fd_fdstat_set_rights: Z$1(function (a, b, c) {
        a = d(a, k(0));
      ...
      fd_filestat_get: Z$1(function (a, b) {
        a = d(a, W$1);
        ...
        b += 8;
        g.view.setBigUint64(b, Y$1(c.atimeMs), !0);
        b += 8;
        g.view.setBigUint64(b, Y$1(c.mtimeMs), !0);
        g.view.setBigUint64(b + 8, Y$1(c.ctimeMs), !0);
        return 0;
      }),
      fd_filestat_set_size: Z$1(function (a, b) {
        a = d(a, Ib$1);
        p.ftruncateSync(a.real, Number(b));
        return 0;
      }),
      fd_filestat_set_times: Z$1(function (a, c, e, g) {
        a = d(a, Jb$1);
      ...
      fd_prestat_get: Z$1(function (a, b) {
        a = d(a, k(0));
        if (!a.path) return 28;
        g.refreshMemory();
        g.view.setUint8(b, 0);
        g.view.setUint32(b + 4, E$1.byteLength(a.fakePath), !0);
        return 0;
      }),
      fd_prestat_dir_name: Z$1(function (a, b, c) {
        a = d(a, k(0));
        if (!a.path) return 28;
        g.refreshMemory();
        E$1.from(g.memory.buffer).write(a.fakePath, b, c, "utf8");
        return 0;
      }),
      fd_pwrite: Z$1(function (a, b, c, f, h) {
        var t = d(a, T | Q$1),
            q = 0;
      ...
      fd_pread: Z$1(function (a, b, c, f, h) {
        var t;
        a = d(a, P$1 | Q$1);
        var q = 0;
      ...
      fd_readdir: Z$1(function (a, b, c, e, f) {
        a = d(a, yb$1);
        g.refreshMemory();
        ...
        for (e = Number(e); e < t.length; e += 1) {
          ...
          b += 3;
          if (b + x >= q + c) break;
          E$1.from(g.memory.buffer).write(h.name, b);
          b += x;
        }
      ...
      fd_sync: Z$1(function (a) {
        a = d(a, S);
        p.fsyncSync(a.real);
        return 0;
      }),
      path_create_directory: Z$1(function (a, b, c) {
        a = d(a, tb$1);
        if (!a.path) return 28;
        g.refreshMemory();
        b = E$1.from(g.memory.buffer, b, c).toString();
        p.mkdirSync(y.resolve(a.path, b));
        return 0;
      }),
      path_filestat_get: Z$1(function (a, b, c, e, f) {
        a = d(a, Cb$1);
        if (!a.path) return 28;
        g.refreshMemory();
        c = E$1.from(g.memory.buffer, c, e).toString();
        c = p.statSync(y.resolve(a.path, c));
        g.view.setBigUint64(f, k(c.dev), !0);
        f += 8;
        g.view.setBigUint64(f, k(c.ino), !0);
        f += 8;
        g.view.setUint8(f, cc$1(g, void 0, c).filetype);
        f += 8;
        g.view.setBigUint64(f, k(c.nlink), !0);
        f += 8;
        g.view.setBigUint64(f, k(c.size), !0);
        f += 8;
        g.view.setBigUint64(f, Y$1(c.atimeMs), !0);
        f += 8;
        g.view.setBigUint64(f, Y$1(c.mtimeMs), !0);
        g.view.setBigUint64(f + 8, Y$1(c.ctimeMs), !0);
        return 0;
      }),
      path_filestat_set_times: Z$1(function (a, c, e, f, h, l, n) {
        a = d(a, Eb$1);
        if (!a.path) return 28;
        g.refreshMemory();
        ...
        e = E$1.from(g.memory.buffer, e, f).toString();
        p.utimesSync(y.resolve(a.path, e), new Date(c), new Date(t));
        return 0;
      }),
      path_link: Z$1(function (a, b, c, e, f, h, l) {
        a = d(a, vb$1);
        f = d(f, wb$1);
        if (!a.path || !f.path) return 28;
        g.refreshMemory();
        c = E$1.from(g.memory.buffer, c, e).toString();
        h = E$1.from(g.memory.buffer, h, l).toString();
        p.linkSync(y.resolve(a.path, c), y.resolve(f.path, h));
        return 0;
      }),
      path_open: Z$1(function (a, b, c, e, f, h, l, n, r) {
        b = d(a, xb$1);
        h = k(h);
        l = k(l);
        a = (h & (P$1 | yb$1)) !== k(0);
        var t = (h & (O$1 | T | sb$1 | Ib$1)) !== k(0);
        if (t && a) var q = p.constants.O_RDWR;else a ? q = p.constants.O_RDONLY : t && (q = p.constants.O_WRONLY);
        a = h | xb$1;
        h |= l;
        0 !== (f & 1) && (q |= p.constants.O_CREAT, a |= ub$1);
        0 !== (f & 2) && (q |= p.constants.O_DIRECTORY);
        0 !== (f & 4) && (q |= p.constants.O_EXCL);
        0 !== (f & 8) && (q |= p.constants.O_TRUNC, a |= Db$1);
        0 !== (n & 1) && (q |= p.constants.O_APPEND);
        0 !== (n & 2) && (q = p.constants.O_DSYNC ? q | p.constants.O_DSYNC : q | p.constants.O_SYNC, h |= O$1);
        ...
        t && 0 === (q & (p.constants.O_APPEND | p.constants.O_TRUNC)) && (h |= Q$1);
        g.refreshMemory();
        c = E$1.from(g.memory.buffer, c, e).toString();
      ...
      path_readlink: Z$1(function (a, b, c, e, f, h) {
        a = d(a, zb$1);
        if (!a.path) return 28;
        g.refreshMemory();
        b = E$1.from(g.memory.buffer, b, c).toString();
        ...
        b = p.readlinkSync(b);
        e = E$1.from(g.memory.buffer).write(b, e, f);
        g.view.setUint32(h, e, !0);
        return 0;
      }),
      path_remove_directory: Z$1(function (a, b, c) {
        a = d(a, Lb$1);
        if (!a.path) return 28;
        g.refreshMemory();
        b = E$1.from(g.memory.buffer, b, c).toString();
      ...
      path_rename: Z$1(function (a, b, c, e, f, h) {
        a = d(a, Ab$1);
        e = d(e, Bb$1);
        if (!a.path || !e.path) return 28;
        g.refreshMemory();
        b = E$1.from(g.memory.buffer, b, c).toString();
        f = E$1.from(g.memory.buffer, f, h).toString();
        p.renameSync(y.resolve(a.path, b), y.resolve(e.path, f));
        return 0;
      }),
      path_symlink: Z$1(function (a, b, c, e, f) {
        c = d(c, Kb$1);
        if (!c.path) return 28;
        g.refreshMemory();
        a = E$1.from(g.memory.buffer, a, b).toString();
        e = E$1.from(g.memory.buffer, e, f).toString();
        p.symlinkSync(a, y.resolve(c.path, e));
        return 0;
      }),
      path_unlink_file: Z$1(function (a, b, c) {
        a = d(a, Mb$1);
        if (!a.path) return 28;
        g.refreshMemory();
        b = E$1.from(g.memory.buffer, b, c).toString();
  ...
  a.prototype.getImportNamespace = function (a) {
    ...
    try {
      for (var e = ca$1(WebAssembly.Module.imports(a)), f = e.next(); !f.done; f = e.next()) {
        var g = f.value;
...
var performance$1 = la.performance || {},
    Ab = performance$1.now || performance$1.mozNow || performance$1.msNow || performance$1.oNow || performance$1.webkitNow || function () {
  return new Date().getTime();
},
    Bb = new Date(),
    Cb = {
  ...
  hrtime: function (a) {
    var b = .001 * Ab.call(performance$1),
        c = Math.floor(b);
    b = Math.floor(b % 1 * 1E9);
    a && (c -= a[0], b -= a[1], 0 > b && (c--, b += 1E9));
    return [c, b];
...
// SPDX-License-Identifier: GPL-2.0-or-later
async function instantiateWasiModule(url, memory, imports = {}) {
  const wasmfs = new sf();
  const wasi = new dc$1({
    preopens: {
      '/': '/'
    },
    bindings: { ...dc$1.defaultBindings,
      fs: wasmfs.fs
    }
  });
...
class WebAGC {
  constructor({
    ...
    this.url = new URL(new URL('assets/yaAGC-cca980ba.wasm', import.meta.url).href, import.meta.url);
    this.data = {
      ports: {
        proceedKey: 0o32,
        normalKey: 0o15
    ...
    this.readyPromise = this.load().then(() => this.configure());
  }
  ...
  async load() {
    ...
    const url = new URL(new URL('assets/yaAGC-cca980ba.wasm', import.meta.url).href, import.meta.url);
    const {
      wasmfs,
      instance
    } = await instantiateWasiModule(url, this.mem, imports);
  ...
  version() {
    ...
    if (typeof versionFunction !== 'function') return 'unknown';
  ...
  */
  ...
  getErasable() {
    if (this.erasableArray) return this.erasableArray;
  ...
  stepCpu(steps) {
    this.instance.exports.cpu_step(steps); // overflow in approx. Number.MAX_SAFE_INTEGER * cycleMs / 1000 / 3600 / 24 / 365 = 3347 years
  ...
  readAllIo() {
    ...
    do {
      ...
      if (channel === 0o11) {
        const bitmask = 0b110; // Bit 2: COMP ACTY
        // Bit 3: UPLINK ACTY
        ...
        const bitmask = 0b111111000; // Bit 1: AGC warning
        // Bit 4: TEMP lamp
        // Bit 5: KEY REL lamp
...
const labelsHardware = {
  ...
  0o01: {
    short: 'L',
    long: 'Low order product of the Accumulator'
  },
  0o02: {
    short: 'Q',
    long: 'TC instruction return address'
  },
  0o03: {
    short: 'EBANK',
    long: 'Erasable storage banking register'
  },
  0o04: {
    short: 'FBANK',
    long: 'Fixed storage banking register'
  },
  0o05: {
    short: 'Z',
    long: '12 bit program counter'
  },
  0o06: {
    short: 'BBANK',
    long: 'Both Banks Register'
  },
  0o07: {
    short: 'ZERO',
    long: 'Hardwired to the value zero'
  },
  0o10: {
    short: 'ARUPT',
    long: 'Interrupted contents of A'
  },
  0o11: {
    short: 'LRUPT',
    long: 'Interrupted contents of L'
  },
  0o12: {
    short: 'QRUPT',
    long: 'Interrupted contents of Q'
  },
  0o13: {
    short: 'SAMPT1',
    long: 'Sampled time 1'
  ...
  0o15: {
    short: 'ZRUPT',
    long: 'Interrupted contents of Z (hardware)'
  },
  0o16: {
    short: 'BANKRUPT',
    long: 'Interrupted contents of BBANK'
  },
  0o17: {
    short: 'BRUPT',
    long: 'Interrupted contents of B internal register'
  },
  0o20: {
    short: 'CYR',
    long: 'Cycle Right'
  ...
  0o30: {
    short: 'TIME5',
    long: 'T5RUPT: Digital Autopilot'
  },
  0o31: {
    short: 'TIME6',
    long: 'T6RUPT: Fine scale clocking'
  }
};
const labelsLM = { ...JSON.parse(JSON.stringify(labelsHardware)),
  // deep clone
  0o23: {
    short: 'EDOP',
    long: 'Edits Interpretative Operation Code Pairs'
  },
  0o32: {
    short: 'CDUX',
    long: 'Inner IMU Gimbal'
  },
  0o33: {
    short: 'CDUY',
    long: 'Middle IMU Gimbal'
  },
  0o34: {
    short: 'CDUZ',
    long: 'Outer IMU Gimbal'
  },
  0o35: {
    short: 'CDUT',
    long: 'Rendevouz Radar Trunnion CDU'
  },
  0o36: {
    short: 'CDUS',
    long: 'Rendevouz Radar Shaft CDU'
  },
  0o37: {
    short: 'PIPAX',
    long: 'Velocity measurement - X axis'
  },
  0o40: {
    short: 'PIPAY',
    long: 'Velocity measurement - Y axis'
  },
  0o41: {
    short: 'PIPAZ',
    long: 'Velocity measurement - Z axis'
  },
  0o42: {
    short: 'BMAGX',
    long: 'RHC input - Pitch'
  },
  0o43: {
    short: 'BMAGY',
    long: 'RHC input - Yaw'
  ...
  0o45: {
    short: 'INLINK',
    long: 'Telemetry Uplink'
  },
  0o46: {
    short: 'RNRAD',
    long: 'Rendevouz and Landing Radar Data'
  },
  0o47: {
    short: 'GYROCTR',
    long: 'Outcounter for gyro'
  },
  0o50: {
    short: 'CDUXMCD',
    long: 'Outcounters for X CDU'
  },
  0o51: {
    short: 'CDUYMCD',
    long: 'Outcounters for Y CDU'
  },
  0o52: {
    short: 'CDUZMCD',
    long: 'Outcounters for Z CDU'
  },
  0o53: {
    short: 'CDUTCMD',
    long: 'Outcounters for Trunnion CDU'
  },
  0o54: {
    short: 'CDUSCMD',
    long: 'Outcounters for Shaft CDU'
  },
  0o55: {
    short: 'THRUST',
    long: 'LM DPS Thrust Command'
  },
  0o56: {
    short: 'LEMONM',
    long: 'Unused'
...
const labelsCM = { ...JSON.parse(JSON.stringify(labelsLM)),
  // deep clone
  ...
  0o53: {
    short: 'OPTYCMD',
    long: 'Outcounter for Optics (Y), SPS Yaw Command in TVC Mode (TVCYAW)'
  },
  0o54: {
    short: 'OPTXCMD',
    long: 'Outcounter for Optics (X), SPS Pitch Command in TVC Mode (TVCPITCH)'
  }
};
var registerSpec = {
...
var _valueEls = /*#__PURE__*/new WeakMap();

var _renderCount = /*#__PURE__*/new WeakMap();

class AGCErasableMemory extends HTMLElement {
  constructor() {
    super();
    ...
    _classPrivateFieldSet(this, _valueEls, new Array(numWordsPerRow * numRows * numBanks));
  }
  ...
  connectedCallback() {
    ...
    for (let bankIdx = 0; bankIdx < numBanks; bankIdx++) {
      ...
      const registerDescriptionEl = createChild(bankEl, 'p', {
        classes: ['register_label']
      }); // Create header row

      const headerRow = createChild(bankEl, 'div');
      createChild(headerRow, 'div', {
        classes: ['erasable_row_label']
      }); // top left spacer

      for (let colIdx = 0; colIdx < numWordsPerRow; colIdx++) {
        const colHeader = createChild(headerRow, 'div', {
          classes: ['col_header']
        });
      ...
      for (let rowIdx = 0; rowIdx < numRows; rowIdx++) {
        ...
        for (let wordIdx = 0; wordIdx < numWordsPerRow; wordIdx++) {
          ...
          const valueEl = createChild(wordEl, 'div', {
            classes: ['value']
          });
          ...
          _classPrivateFieldGet(this, _valueEls)[address] = textEl;
        }
      }
    }
  }
  ...
  render() {
    var _this$renderCount;

    if (this.stopping) return;
    window.requestAnimationFrame(() => this.render());
    if ((_classPrivateFieldSet(this, _renderCount, (_this$renderCount = +_classPrivateFieldGet(this, _renderCount)) + 1), _this$renderCount) % 3) return;

    for (let idx = 0; idx < this.memLength; idx++) {
      const wordEl = _classPrivateFieldGet(this, _valueEls)[idx];
...
var channelSpec = {
  ...
  LM: {
    0o5: {
      output: true,
      labels: ['RCS Jet B4U (1)', 'RCS Jet A4D (2)', 'RCS Jet A3U (5)', 'RCS Jet B3D (6)', 'RCS Jet B2U (9)', 'RCS Jet A2D (10)', 'RCS Jet A1U (13)', 'RCS Jet B1D (14)', 'not used', 'not used', 'not used', 'not used', 'not used', 'not used', 'not used']
    },
    0o6: {
      output: true,
      labels: ['RCS Jet B3A (7)', 'RCS Jet B4F (3)', 'RCS Jet A1F (15)', 'RCS Jet A2A (1)', 'RCS Jet B2L (12)', 'RCS Jet A3R (8)', 'RCS Jet A4R (4)', 'RCS Jet B1L (16)', 'not used', 'not used', 'not used', 'not used', 'not used', 'not used', 'not used']
    },
    0o10: {
      output: true,
      labels: ['DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit Selector', 'DSKY Digit Selector', 'DSKY Digit Selector', 'DSKY Digit Selector']
    },
    0o11: {
      output: true,
      labels: ['ISS Warning', 'Computer Activity Lamp', 'Uplink Activity Lamp', 'Temperature Caution Lamp', 'Keyboard Release Lamp', 'Flash Verb and Noun Lamps', 'Operator Error Lamp', 'Spare', 'Test Connector Outbit', 'Caution Reset', 'Spare', 'Spare', 'Eng. On', 'Eng. Off', 'Spare']
    },
    0o12: {
      output: true,
      labels: ['Zero RR CDU\'s', 'Enable RR Error Counters', 'Not Used', 'Coarse Align Enable', 'Zero IMU CDU\'s', 'Enable IMU Error Counters', 'Spare', 'Display Inertial Data', '+ Pitch Vehicle Motion', // (Pitch Gimbal Trim, Bell motion)',
      '- Pitch Vehicle Motion', // (Pitch Gimbal Trim, Bell motion)',
      '+ Roll Vehicle Motion', // (Pitch Gimbal Trim, Bell motion)',
      '- Roll Vehicle Motion', // (Pitch Gimbal Trim, Bell motion)',
      '', // omitted?
      'RR Enable Auto Track', 'ISS Turn-on Delay Compl']
    },
    0o13: {
      output: true,
      labels: ['Radar Select c', 'Radar Select b', 'Radar Select a', 'Radar Activity', 'Inh. Uplink, En. Crosslnk', 'Block Inlink', 'Downlink Word Order', 'RHC Counter Enable', 'Start RHC Read', 'Test Alarms', 'Enable Standby', 'Reset Trap 31-A', 'Reset Trap 31-B', 'Reset Trap 32', 'Enable T6RUPT']
    },
    0o14: {
      output: true,
      labels: ['Outlink Activity', 'Altitude Rate Select', 'Altitude Meter Activity', 'Thrust Drive', 'Spare', 'Gyro Enable', 'Gyro Select b', 'Gyro Select a', 'Gyro Sign Minus', 'Gyro Activity', 'Drive CDU S', 'Drive CDU T', 'Drive CDU Z', 'Drive CDU Y', 'Drive CDU X']
    },
    0o15: {
      output: false,
      labels: ['Main DSKY key code', 'Main DSKY key code', 'Main DSKY key code', 'Main DSKY key code', 'Main DSKY key code', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare']
    },
    0o16: {
      output: false,
      labels: ['Spare', 'Spare', 'Mark X', 'Mark Y', 'Mark Reject', 'Descent +', 'Descent -', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare']
    },
    0o30: {
      output: false,
      labels: ['Abort with Descent Stage', 'Descent Stage Attached', 'Eng. Armed', 'Abort with Ascent Stage', 'Auto Throttle', 'Display Inertial Data', 'RR CDU Fail', 'Spare', 'IMU Operate', 'G/N Control of S/C', 'IMU Cage', 'IMU CDU Fail', 'IMU Fail', 'ISS Turn-On Request', 'Temp in Limits']
    },
    0o31: {
      output: false,
      labels: ['+ EL (LPD), + PMI', '- EL (LPD), + PMI', '+ YMI', '- YMI', '+ AZ (LPD), + RMI', '- AZ (LPD), + RMI', '+X Translation', '-X Translation', '+Y Translation', '-Y Translation', '+Z Translation', '-Z Translation', 'Att. Hold', 'Auto Stabilization', 'Att. Control Detent']
    },
    0o32: {
      output: false,
      labels: ['Thruster 2-4 Disable', 'Thruster 5-8 Disable', 'Thruster 1-3 Disable', 'Thruster 6-7 Disable', 'Thruster 14-16 Disable', 'Thruster 13-15 Disable', 'Thruster 9-12 Disable', 'Thruster 10-11 Disable', 'Desc. Eng. Gimbals Dis.', 'Desc. Eng. Gimbal Fail', 'Spare', 'Spare', 'Spare', 'Proceed', 'Spare']
    },
    0o33: {
      output: false,
      labels: ['Spare', 'RR Auto-Power On', 'RR Range Low Scale', 'RR Data Good', 'LR Range Data Good', 'LR Position 1', 'LR Position 2', 'LR Velocity Data Good ', 'LR Range Low Scale', 'Block Uplink Input', 'Uplink Too Fast', 'Downlink Too Fast', 'PIPA Fail', 'LGC Warning', // (repeated hardware or software alarms)',
      'Oscillator Alarm']
    },
    0o77: {
      output: false,
      labels: ['E or F Memory Parity Fail', 'E Memory Parity Fail', 'TC Trap', 'RUPT Lock', 'Night Watchman', 'Voltage Fail', 'Counter Fail', 'Scaler Fail', 'Scaler Double Frequency', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare']
    }
  },
  CM: {
    0o5: {
      output: true,
      labels: ['C-3/I-3', 'C-4/2-4', 'A-3/2-3', 'A-4/1-4', 'D-3/2-5', 'D-4/1-6', 'B-3/1-5', 'B-4/2-6', 'not used', 'not used', 'not used', 'not used', 'not used', 'not used', 'not used']
    },
    0o6: {
      output: true,
      labels: ['B-1/1-1', 'B-2/1-2', 'D-1/2-1', 'D-2/2-2', 'A-1', 'A-2', 'C-1', 'C-2', 'not used', 'not used', 'not used', 'not used', 'not used', 'not used', 'not used']
    },
    0o10: {
      output: true,
      labels: ['DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit', 'DSKY Digit']
    },
    0o11: {
      output: true,
      labels: ['ISS Warning', 'Computer Activity Lamp', 'Uplink Activity Lamp', 'Temperature Caution Lamp', 'Keyboard Release Lamp', 'Flash Verb and Noun Lamps', 'Operator Error Lamp', 'Spare', 'Test Connector Outbit', 'Caution Reset', 'Spare', 'Spare', 'Eng. On/Off', 'Spare', 'Spare']
    },
    0o12: {
      output: true,
      labels: ['Zero Optics CDU\'s', 'En. Optics COU Err Cntrs', 'Not Used', 'Coarse Align Enable', 'Zero IMU CDU\'s', 'En. IMU Err Cntrs', 'Spare', 'TVC Enable', 'Enable S-IVB Takeover', 'Zero Optics', 'Disengage Optics DAC', 'Spare', 'S-IVB Injection Seq Start', 'S-IVB Cutoff', 'ISS Turn-on Delay Compl']
    },
    0o13: {
      output: true,
      labels: ['Range Unit Select c', 'Range Unit Select b', 'Range Unit Select a', 'Range Unit Activity', 'Not Used', 'Block Inlink', 'Downlink Word Order Code Bit', 'Not Used', 'Spare', 'Test Alarms', 'Enable Standby', 'Reset Trap 31-A', 'Reset Trap 31-B', 'Reset Trap 32', 'Enable T6RUPT']
    },
    0o14: {
      output: true,
      labels: ['Not Used', 'Spare', 'Spare', 'Spare', 'Not Used', 'Gyro Enable', 'Gyro Select b', 'Gyro Select a', 'Gyro Sign (1 = minus)', 'Gyro Activity', 'Drive CDU S', 'Drive CDU T', 'Drive CDU Z', 'Drive CDU Y', 'Drive CDU X']
    },
    0o15: {
      output: false,
      labels: ['Main DSKY key code', 'Main DSKY key code', 'Main DSKY key code', 'Main DSKY key code', 'Main DSKY key code', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare']
    ...
    0o16: {
      output: false,
      labels: ['Nav DSKY key code', 'Nav DSKY key code', 'Nav DSKY key code', 'Nav DSKY key code', 'Nav DSKY key code', 'Mark Button', 'Mark Reject Button', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare']
    },
    0o30: {
      output: false,
      labels: ['Ullage Thrust Percent', 'CM/SM Separate', 'SPS Ready', 'S-IVB Separate, Abort', 'Liftoff', 'Guidance Reference Release', 'Optics CDU Fail', 'Spare', 'IMU Operate', 'S/C Control of Saturn', 'IMU Cage', 'IMU CDU Fail', 'IMU Fail', 'ISS Turn-On Request', 'Temp in Limits']
    },
    0o31: {
      output: false,
      labels: ['+ Pitch Manual Rotation', '- Pitch Manual Rotation', '+ Yaw Manual Rotation', '- Yaw Manual Rotation', '+ Roll Manual Rotation', '- Roll Manual Rotation', '+X Translation', '-X Translation', '+Y Translation', '-Y Translation', '+Z Translation', '-Z Translation', 'Hold Function', 'Free Function', 'G&N Autopilot Control']
    },
    0o32: {
      output: false,
      labels: ['+ Pitch Minimum Impulse', '- Pitch Minimum Impulse', '+ Yaw Minimum Impulse', '- Yaw Minimum Impulse', '+ Roll Minimum Impulse', '- Roll Minimum Impulse', 'Spare', 'Spare', 'Spare', 'Spare', 'LM Attached', 'Spare', 'Spare', 'Proceed (Standby button)', 'Spare']
    },
    0o33: {
      output: false,
      labels: ['Spare', 'Range Unit Data Good', 'Spare', 'Zero Optics', 'CMC Control', 'Not Used', 'Not Used', 'Spare', 'Spare', 'Block Uplink Input', 'Uplink Too Fast', 'Downlink Too Fast', 'PIPA Fail', 'AGC Warning', 'AGC Oscillator Alarm']
    },
    0o77: {
      output: false,
      labels: ['E or F Memory Parity Fail', 'E Memory Parity Fail', 'TC Trap', 'RUPT Lock', 'Night Watchman', 'Voltage Fail', 'Counter Fail', 'Scaler Fail', 'Scaler Double Freq Alarm', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare', 'Spare']
    }
  }
};

var _bitEls = /*#__PURE__*/new WeakMap();

var _updateBitTimeout = /*#__PURE__*/new WeakMap();

var _toggleBit = /*#__PURE__*/new WeakSet();

var _updateBits = /*#__PURE__*/new WeakSet();

class AGCChannel extends HTMLElement {
  constructor() {
    super();

    _updateBits.add(this);
    ...
    createChild(this.rootEl, 'link', {
      attributes: {
        rel: 'stylesheet',
        href: new URL(new URL('assets/agc_channel-6b83a4dd.css', import.meta.url).href, import.meta.url)
      }
    });
  }
  ...
  connectedCallback() {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      const bitEl = createChild(rowEl, 'div', {
        classes: ['bit']
      });
      const labelEl = createChild(rowEl, 'label');
      _classPrivateFieldGet(this, _bitEls)[i] = bitEl;

      if (this.getAttribute('data-direction') === 'input') {
        rowEl.addEventListener('mousedown', () => _classPrivateMethodGet(this, _toggleBit, _toggleBit2).call(this, i));
        rowEl.addEventListener('mouseup', () => _classPrivateMethodGet(this, _toggleBit, _toggleBit2).call(this, i));
        rowEl.addEventListener('dblclick', () => _classPrivateMethodGet(this, _toggleBit, _toggleBit2).call(this, i));
      }
  ...
  attributeChangedCallback(attrName, _, newVal) {
    if (attrName === 'data-state') {
      ...
      if (_classPrivateFieldGet(this, _updateBitTimeout)) return;

      _classPrivateFieldSet(this, _updateBitTimeout, setTimeout(() => {
        _classPrivateFieldSet(this, _updateBitTimeout, null);

        _classPrivateMethodGet(this, _updateBits, _updateBits2).call(this);
...
function _toggleBit2(bitNumber) {
...
function _updateBits2() {
  for (let i = 0; i < 15; i++) _classPrivateFieldGet(this, _bitEls)[i].setAttribute('data-value', !!(this.value & 1 << i));
}

customElements.define('agc-channel', AGCChannel);

var _configureInputElement = /*#__PURE__*/new WeakSet();

var _handleKeyboardKey = /*#__PURE__*/new WeakSet();

// SPDX-License-Identifier: GPL-2.0-or-later
// SPDX-FileCopyrightText: Copyright 2020 Michael Karl Franzl <public.michael@franzl.name>
class DskyPcKeyboardInterface {
  constructor(dsky) {
    _handleKeyboardKey.add(this);

    _configureInputElement.add(this);

    this.dsky = dsky;
    this.inputElement = document.getElementById('dsky_key_input');

    _classPrivateMethodGet(this, _configureInputElement, _configureInputElement2).call(this);
  }
  ...
  type(string, index = 0) {
    ...
    _classPrivateMethodGet(this, _handleKeyboardKey, _handleKeyboardKey2).call(this, char); // keydown now


    setTimeout(() => _classPrivateMethodGet(this, _handleKeyboardKey, _handleKeyboardKey2).call(this, ''), 200); // keyup a bit later
...
function _configureInputElement2() {
  ...
  this.inputElement.addEventListener('keydown', event => {
    ...
    _classPrivateMethodGet(this, _handleKeyboardKey, _handleKeyboardKey2).call(this, event.key); // handle single character key strokes
  ...
  this.inputElement.addEventListener('paste', event => {
    ...
    for (const item of event.clipboardData.items) if (item.type === 'text/plain') item.getAsString(string => this.type(string));
  });
}

function _handleKeyboardKey2(key) {
  if (!key) this.dsky.setAttribute('data-keypress', ''); // keyup
  // Prevent entering any non supported character.

  if (!/^[vn+-cpker0-9\s]$/.test(key.toLowerCase())) return; // Let's do some key mapping because webDSKY only takes alphanumerical key strokes.
  ...
  if (dskyKey !== ' ') this.dsky.setAttribute('data-keypress', dskyKey);
}
...
var _clockDivisorElement = /*#__PURE__*/new WeakMap();

var _showVersion = /*#__PURE__*/new WeakSet();

var _loadProgram = /*#__PURE__*/new WeakSet();

var _runProgram = /*#__PURE__*/new WeakSet();

var _createAGC = /*#__PURE__*/new WeakSet();

var _renderIOChannels = /*#__PURE__*/new WeakSet();

var _createDSKY = /*#__PURE__*/new WeakSet();

var _configureClockDivisor = /*#__PURE__*/new WeakSet();

var _configureAGCProgramSelector = /*#__PURE__*/new WeakSet();

var _configureCPUManipulationButtons = /*#__PURE__*/new WeakSet();

var _getClockDivisor = /*#__PURE__*/new WeakSet();

class Demo {
  constructor() {
    _getClockDivisor.add(this);

    _configureCPUManipulationButtons.add(this);

    _configureAGCProgramSelector.add(this);

    _configureClockDivisor.add(this);

    _createDSKY.add(this);

    _renderIOChannels.add(this);

    _createAGC.add(this);

    _runProgram.add(this);

    _loadProgram.add(this);

    _showVersion.add(this);

    _clockDivisorElement.set(this, {
      writable: true,
      value: document.getElementById('clock_divisor')
    });

    _classPrivateMethodGet(this, _createDSKY, _createDSKY2).call(this);

    this.dskyPcKeyboardInterface = new DskyPcKeyboardInterface(this.dsky);

    _classPrivateMethodGet(this, _createAGC, _createAGC2).call(this);
  }
  ...
  async run() {
    ...
    _classPrivateMethodGet(this, _configureClockDivisor, _configureClockDivisor2).call(this);

    _classPrivateMethodGet(this, _configureAGCProgramSelector, _configureAGCProgramSelector2).call(this);

    _classPrivateMethodGet(this, _configureCPUManipulationButtons, _configureCPUManipulationButtons2).call(this);
...
async function _loadProgram2(url) {
  const response = await fetch(url);
  const binary = await response.arrayBuffer();

  _classPrivateMethodGet(this, _runProgram, _runProgram2).call(this, binary);
}

async function _runProgram2(binary) {
  this.agc.loadRom(binary);
  this.agc.reset();
}

function _createAGC2() {
  this.agc = new WebAGC({
    ...
    onChannelUpdate: (channelNumber, value) => {
      switch (channelNumber) {
        ...
        case 0o11: // Some bits need to be sent to DSKY lights. This is handled by onDSKYLightsUpdate.
...
function _renderIOChannels2(simType) {
  this.channelEls = {};

  for (const el of new Array(...document.getElementsByTagName('agc-channel'))) el.remove();
  ...
  for (const chanNumber of outputChannels) {
    ...
    chanEl.addEventListener('change', ({
      detail
    }) => this.agc.writeIo(chanNumber, detail));
...
function _createDSKY2() {
  ...
  this.dsky.addEventListener('proceed', ({
    detail
  }) => this.agc.proceedKeyPress(detail));
}

function _configureClockDivisor2() {
  _classPrivateFieldGet(this, _clockDivisorElement).addEventListener('change', () => {
...
function _configureAGCProgramSelector2() {
  ...
  document.getElementById('agc_program_selector').addEventListener('change', event => {
    ...
    let simulationType;
    if (programName.includes('Luminary')) simulationType = 'LM';else if (programName.includes('Comanche')) simulationType = 'CM';else simulationType = 'other';

    for (const el of new Array(...document.getElementsByTagName('agc-erasable-memory'))) el.remove();
    ...
    _classPrivateMethodGet(this, _renderIOChannels, _renderIOChannels2).call(this, simulationType);

    _classPrivateMethodGet(this, _loadProgram, _loadProgram2).call(this, `agc/${programName}`);
  });
}

function _configureCPUManipulationButtons2() {
  ...
  document.getElementById('button_run').addEventListener('click', () => {
    this.agc.oscillate(_classPrivateMethodGet(this, _getClockDivisor, _getClockDivisor2).call(this));
    document.getElementById('agc_state').innerHTML = 'running';
  });
...
function _getClockDivisor2() {
  switch (_classPrivateFieldGet(this, _clockDivisorElement).value) {
    case 'inf':
      return Infinity;

    default:
      return parseInt(_classPrivateFieldGet(this, _clockDivisorElement).value, 10);
  }
}
...
function getBrowserInfo() {
  ...
  if (navigator.userAgent.indexOf('Linux') !== -1) result.os = 'Linux';else if (navigator.userAgent.indexOf('Windows') !== -1) result.os = 'Windows';else if (navigator.userAgent.indexOf('Macintosh') !== -1) result.os = 'Mac';
  return result;
}

window.addEventListener('load', async () => {
  const browserInfo = getBrowserInfo();
  if (browserInfo.name === 'Firefox' && browserInfo.version < 78 || browserInfo.name.startsWith('Chrom') && browserInfo.version < 89) document.getElementById('errors').innerHTML = 'This browser is too old. The application may not run correctly, or at all. Requiring at least Firefox version 78 or Chrome version 89.';
  const demo = new Demo();
  demo.run();
});
// This file was generated using the build script of webAGC. See https://github.com/michaelfranzl/webAGC
